...
public class CompassView extends View
{
    ...
    protected void initCompassView() {
        ...
        textPaint.setSubpixelText(true);// 字体抗锯齿
    ...
    private int measure(int measureSpec) {
        ...
        if(specMode == MeasureSpec.UNSPECIFIED){
            result=100;// 未定义返回两百像素
        }
        else{
            // 因为我们希望填充父ViewGroup给的大小空间
            // 所以返回实际大小即可
            result=specSize;
        }
        return result;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int px=width / 2;
        int py=height / 2;
        Point center=new Point(px, py);// 原心位置
        int radius=Math.min(px, py) - 2;// 外圆半径
        // 外正方形
        ...
        RectF innerBoundingBox=new RectF(center.x - radius + ringWidth,
                                         ...
                                         center.y + radius - ringWidth);
        float innerRadius=innerBoundingBox.height() / 2;// 内圆半径
        // 外圆的辐射渐变shader
        ...
        canvas.drawPath(outerRingPath, pgb);// 画出外圆
        // 天空线性渐变的shader
        ...
        // ---------------
        int markWidth=radius / 3;// 横线刻度的长度
